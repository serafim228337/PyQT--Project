...
class TerrariaCraftingApp(QWidget):
    def __init__(self, user_id):
        ...
        self.craft_input = QLineEdit(keys[0])
        ...
        grid.addWidget(self.materials_list, 5, 0, 1, 3)  # Разместили список материалов
    ...
    def update_favorite_button(self):
        selected_item = self.recipe_list.currentText()
        if selected_item in self.favorite_recipes:
            self.favorite_button.setText("Избранное (✔)")  # Или добавьте звездочку
        else:
            self.favorite_button.setText("Избранное")

    def save_favorite_recipes(self):
        # Очистка таблицы избранных рецептов для данного пользователя
        self.cursor.execute("DELETE FROM favorite_recipes WHERE user_id = ?", (self.user_id,))

        # Вставка новых избранных рецептов в базу данных
        for recipe_name in self.favorite_recipes:
            self.cursor.execute("INSERT INTO favorite_recipes (user_id, recipe_name) VALUES (?, ?)",
                                (self.user_id, recipe_name))
